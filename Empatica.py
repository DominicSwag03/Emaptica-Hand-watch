...
outputFile = "mergedBioData3.csv" #Setting name of output file
    ...
    with open(file, 'r') as csvfile:
        ...
        for row in reader:
            if(i == 0):
                ...
                print ("Initial Timestamp ",initialTimestamp)
                print (" exact Time: ",convertMilisToTime(initialTimestamp))
            else:
...
with open(outputFile,'w') as f1:
    ...
    writer.writerow(row)    
    for timestampEDA, eda in EDA.items():
        timestamp = convertMilisToTime(timestampEDA)
        ...
        meanHR = 0.0
        while i < 4.0:
            if (timestampEDA + i in EDA):
                total = total + float(EDA[timestampEDA+i])
                count = count+1
            i = i + 4.0/HRHertz
        if(count > 0):
            meanEDA = total/count
        print ("Merging HRV and EDA at ", timestamp),
        print(" edaV: ",eda)
        print(" HR ",meanHR)
        print(" count: ",count)
        #merging with BVP 
        ...
        meanBVP = 0.0
        while i < 4.0:
            if (timestampEDA + i in BVP):
                total = total + float(BVP[timestampEDA +i])
                count = count+1
            i = i + 4.0/BVPHertz
        if(count > 0):
            meanBVP = total/count
        print ("Merging HRV and BVP at ", timestamp)
        print( " EDAV: ",eda)
        print(" BVP ",meanBVP)
        print(" count: ",count)
        #merging with TEMP
        ...
        meanTemp = 0.0
        while i < 4.0:
            if (timestampEDA + i in TEMP):
                total = total + float(TEMP[timestampEDA+i])
                count = count+1
            i = i + 4.0/TEMPHertz
        if(count > 0):
            meanTEMP = total/count
        print ("Merging HRV and TEM at ", timestamp)
        print(" EDAV: ",eda)
        (" TEM ",meanTEMP)
        (" count: ",count ) 
        #merging with ACC
        ...
        meanOverall = 0.0
        while i < 4.0:
            if (timestampEDA + i in ACC):
                totalX = totalX + float(ACC[timestampEDA+i]['x'])
                totalY = totalY + float(ACC[timestampEDA+i]['y'])
                totalZ = totalZ + float(ACC[timestampEDA+i]['z'])
                totalOverall = totalOverall + float(ACC[timestampEDA+i]['overall'])
                count = count+1
            i = i + 4.0/ACCHertz
        if(count > 0):
            meanX = totalX/count
        ...
        print ("Merging HRV and ACC at ", timestamp)
        (" EDAV: ",eda)
        (" ACC ",meanOverall)
        (" count: ",count)
        ...
        sumIBI = 0.0
        while i < 1.0:
            if(timestampEDA + i in IBI):
                #print "Timestamps matched-- HR:",timestampHR," IBI: ",timestampHR+i
                total = total + float(IBI[timestampEDA+i])
                count = count + 1
            i = i + 0.1
        if(count > 0):
            sumIBI = total
        print ("Merging HRV and IBI at ", timestamp)
        print(" milis: ",timestampEDA)
        print(" EDAV: ",eda) 
        print(" Sum IBI: ",sumIBI)
        print(" count: ",count)
        
        row = participantID,timestamp,hour[1],eda,meanEDA,meanBVP,meanTEMP,meanX,meanY,meanZ,meanOverall,sumIBI,count
        writer.writerow(row)
print ("--------------------------------------------------------------------------------" )       
